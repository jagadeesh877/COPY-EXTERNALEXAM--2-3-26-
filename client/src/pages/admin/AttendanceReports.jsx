...
const AttendanceReports = () => {
  ...
  const downloadCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Attendance_Report_${department}_${year}_${section}.csv`,
    );
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `Attendance_Report_${department}_${year}_${section}_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
      document.body.appendChild(link);
      link.click();
      link.remove();
  ...
  return (
    <div className="flex flex-col animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6 px-2">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">
            Analyze institutional attendance trends and identify student
      ...
      <div className="bg-white p-10 rounded-[40px] shadow-xl border border-gray-100 mb-10 relative transition-all duration-700 hover:shadow-2xl">
      ...
      {reportData.length > 0 ? (
        <div className="bg-white p-10 rounded-[40px] shadow-xl border border-gray-100 min-h-[500px] transition-all relative overflow-hidden">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-6 relative z-10">
            <div>
              <h3 className="text-2xl font-black text-[#003B73] tracking-tight uppercase">
                Detailed Intelligence Report
              </h3>
              <p className="text-gray-400 font-bold text-xs uppercase tracking-widest mt-1">
                Granular Student Attendance Breakdown
              </p>
            </div>
            <div className="flex gap-4">
              ...
              >
                ...
                {exporting ? "Processing..." : "Excel Intelligence"}
              </button>
            </div>
          </div>
                  ...
                  return (
                    ...
                    >
                      <td className="px-8 py-6 text-left">
                        <span className="font-mono text-xs font-black text-[#003B73] uppercase tracking-tighter">
                          {student.rollNo}
                        </span>
                      </td>
                      <td className="px-8 py-6 text-left">
                      ...
                      <td className="px-8 py-6 text-right">
                        <span
                          className={`px-4 py-2 rounded-xl text-[10px] font-black tracking-widest border ${statusColor} shadow-sm inline-flex items-center gap-2`}
                        >
                          {statusLabel === "CRITICAL" && (
        ...
        <div className="bg-white p-20 rounded-[40px] shadow-xl border border-gray-100 text-center flex flex-col items-center group">
          ...
          <h3 className="text-2xl font-black text-gray-400 tracking-tight uppercase mb-2">
            Awaiting Intelligence
          </h3>
          <p className="text-gray-400 font-medium max-w-sm">
            Use the analytical filters above to generate institutional
            attendance reports.
          </p>
        </div>
      )}
